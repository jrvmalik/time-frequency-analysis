function [cft, frequency] = ConceFT_J4(x, Fs, hlength, iter, hop, n, hf, lf, ths)
...
switch nargin
    ...
    case 0
        ...
        hf = 12;
        ths = 1;
        disp('Testing code on a noisy 2 Hz sawtooth wave.')
end
...
threshold = quantile(abs(tfr), (1 - ths));

% omega operator
neta = length(frequency);
omega = -inf(neta, tcol);
ups = ~bsxfun(@le, abs(tfr), threshold);
